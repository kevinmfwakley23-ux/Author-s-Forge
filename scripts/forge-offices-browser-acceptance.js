...
async function stop(process) {
  ...
  await Promise.race([new Promise((resolve) => process.once("exit", resolve)), sleep(3000)]);
  if (process.exitCode === null) process.kill("SIGKILL");
}

async function main() {
  const dataDir = await mkdtemp(join(tmpdir(), "authors-forge-offices-"));
  const launcher = spawn(process.execPath, ["scripts/start-forge.js", `--host=${HOST}`], {
    env: {
      ...process.env,
      FORGE_DATA_DIR: dataDir,
      PORT: String(PORTS.studio),
      JOURNAL_PORT: String(PORTS.journal),
      WORKBOOK_PORT: String(PORTS.workbooks),
      SPECIALIZED_PORT: String(PORTS.specialized),
      OMNIROUTE_BASE_URL: "",
      ROUTER9_BASE_URL: "",
      KINGS_AI_ENDPOINT: "",
      OPENAI_API_KEY: "",
  ...
  try {
    ...
    const created = await fetch(`http://${HOST}:${PORTS.studio}/api/projects`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ id: projectId, title: "Cross-office acceptance" }),
    });
    ...
    for (const [selector, port, titlePattern] of offices) {
      ...
      const response = await officePage.goto(href, { waitUntil: "domcontentloaded" });
      assert.ok(response?.ok(), `${selector} target must be reachable through the unified launcher.`);
      assert.match(await officePage.locator("body").innerText(), titlePattern);
