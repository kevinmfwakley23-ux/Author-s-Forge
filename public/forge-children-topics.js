...
(() => {
  ...
  function loadCatalog() {
    if (!catalogPromise) {
      catalogPromise = fetch('/children-story-topics.json', { cache: 'no-cache' }).then(async (response) => {
  ...
  function isTopicRequest(instruction) {
    ...
    return audience && subject && action;
  }
  ...
  function flatten(catalog) {
    return catalog.categories.flatMap((category) => category.topics.map((topic) => ({ topic, category: category.label })));
  }

  function render(catalog, instruction) {
    const limit = requestedLimit(instruction);
    const items = flatten(catalog).slice(0, limit);
    const heartwood = /heartwood/i.test(String(instruction || ''));
    const heading = heartwood ? `Heartwood Jungle story-topic list (${items.length})` : `Children's story-topic list (${items.length})`;
    const lines = items.map((item, index) => `${index + 1}. ${item.topic} — ${item.category}`);
    return [
      heading,
      '',
      ...lines,
      '',
      heartwood ? `Heartwood framing: ${catalog.heartwoodGuidance}` : catalog.intendedUse,
      '',
      `Source basis: ${catalog.sourceBasis.map((source) => source.organization).filter((value, index, values) => values.indexOf(value) === index).join(', ')}.`,
    ].join('\n');
  }
  ...
  async function runTopicRequest(instruction, { openCommandCenter = false } = {}) {
    ...
    try {
      ...
      if (status) status.textContent = `Children's story topic list ready — ${requestedLimit(instruction)} topic${requestedLimit(instruction) === 1 ? '' : 's'}, no diagnosis and no provider required.`;
    } catch (error) {
  ...
  function interceptClick(event, source) {
    ...
    if (!isTopicRequest(instruction)) return;
    event.preventDefault();
    event.stopImmediatePropagation();
    void runTopicRequest(instruction, { openCommandCenter: true });
  }
  ...
  function bind() {
    ...
    $('#fcc-command')?.addEventListener('keydown', (event) => {
      ...
      const instruction = $('#fcc-command')?.value || '';
      if (!isTopicRequest(instruction)) return;
      event.preventDefault();
      event.stopImmediatePropagation();
      void runTopicRequest(instruction);
    }, true);
  }
