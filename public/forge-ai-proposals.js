...
(() => {
  ...
  async function loadProposals() { try { proposals = await api(`/api/projects/${encodeURIComponent(projectId)}/ai/proposals`); render(); } catch (error) { notify(error.message); } }
  function render() {
    const host = $("#ai-proposals"); if (!host) return;
    if (!proposals.length) { host.innerHTML = '<p class="muted">No durable AI proposals yet. Run the real AI writing engine to create one.</p>'; return; }
