...
(() => {
  ...
  function defaultImagesRequired(book) { return Boolean(book && ['childrens-book','comic-book'].includes(book.kind)); }
      ...
      office.innerHTML = `
        <div class="grid">
          ...
          <article class="card">
            ...
            <p class="muted">Run readiness separately for each edition. Forge reads actual illustration count/approval from project state; author-entered evidence cannot invent those assets.</p>
            <form id="publishing-readiness-form">
              <label>Edition to audit<select name="releaseFormat" id="publishing-release-format"><option value="ebook">eBook</option><option value="paperback">Paperback</option><option value="hardcover">Hardcover</option></select></label>
              <div class="row"><label>Print page count<input name="pageCount" type="number" min="1"></label><label>Output file types<input name="fileTypes" placeholder="epub or pdf"></label></div>
              <fieldset><legend>Manuscript/front matter</legend><label><input type="checkbox" name="hasTitlePage"> Title page</label><label><input type="checkbox" name="hasCopyrightPage"> Copyright page</label><label><input type="checkbox" name="tableOfContentsRequired" checked> TOC required for this edition</label><label><input type="checkbox" name="hasTableOfContents"> Table of contents present</label><label><input type="checkbox" name="hasDedication"> Dedication</label><label><input type="checkbox" name="hasAboutTheAuthor"> About author</label><label><input type="checkbox" name="hasBackMatter"> Back matter</label></fieldset>
              <fieldset><legend>Final cover evidence</legend><div class="row"><label>File type<input name="coverFileType" placeholder="jpeg or pdf"></label><label>Width (in, print)<input name="coverWidth" type="number" min="0" step="0.001"></label><label>Height (in, print)<input name="coverHeight" type="number" min="0" step="0.001"></label></div><label><input type="checkbox" name="coverFront"> Final front cover present</label><label><input type="checkbox" name="coverBack"> Back cover present</label><label><input type="checkbox" name="coverSpine"> Spine present</label><label><input type="checkbox" name="coverBarcode"> Barcode-safe area validated</label><label><input type="checkbox" name="coverBleed"> Cover bleed validated</label><label><input type="checkbox" name="coverTrim"> Cover trim validated</label><label><input type="checkbox" name="coverMargins"> Cover safe margins validated</label><label><input type="checkbox" name="coverValidated"> Final cover passed validation</label></fieldset>
              <fieldset><legend>Illustrations</legend><label><input type="checkbox" name="imagesRequired"> Images are required for this edition</label><label><input type="checkbox" name="resolutionValidated"> Production resolution validated for the real project illustrations</label><p class="muted">Asset count, URI resolution, and approval status are read from the durable Illustration Library.</p></fieldset>
              <fieldset><legend>Production validation</legend><label><input type="checkbox" name="formattingValidated"> Formatting validated</label><label><input type="checkbox" name="pageNumbering"> Print page numbering configured</label><label><input type="checkbox" name="headersFooters"> Print headers/footers configured or deliberately omitted</label><label><input type="checkbox" name="productionTrim"> Interior trim validated</label><label><input type="checkbox" name="productionBleed"> Interior bleed validated</label><label><input type="checkbox" name="productionValidated"> Final production validation passed</label></fieldset>
      ...
      office.innerHTML = `
        <div class="grid">
          ...
          <article class="card">
            ...
            <div id="promotion-readiness-result" class="list"></div><div id="promotion-campaigns" class="list"></div>
          </article>
        </div>`;
  ...
  function syncBooks() {
    ...
    if (selected) void selectBook(selected, true);
  }
  ...
  async function selectBook(bookId, load = true) {
    ...
    const imagesRequired = $('#publishing-readiness-form')?.elements?.imagesRequired;
    if (imagesRequired) imagesRequired.checked = defaultImagesRequired(activeBook());
    if (!load || !bookId) return;
    await loadPublishingMetadata().catch((error)=>message(error.message));
    await Promise.allSettled([loadMarketResearch(), loadCampaigns(), loadPublishingReadiness()]);
  }

  function syncReleaseFormatFromMetadata(metadata) {
    const select = $('#publishing-release-format'); if (!select) return;
    const formats = metadata?.formats || [];
    const next = formats.includes(select.value) ? select.value : formats[0] || 'ebook';
    if (['ebook','paperback','hardcover'].includes(next)) select.value = next;
  }
  ...
  async function loadPublishingMetadata() {
    ...
    syncReleaseFormatFromMetadata(m); renderCompliance(current);
  }
  ...
  async function savePublishingMetadata(event) {
    ...
    if (!formats.length) return message('Select at least one publishing format.');
    try {
  ...
  async function loadCampaigns(){if(!state.bookId)return;try{const data=await api(rootUrl(`/promotion/campaigns?bookId=${encodeURIComponent(state.bookId)}`));state.campaigns=data.campaigns||[];if(!state.campaigns.some((entry)=>entry.campaign?.id===state.activeCampaignId))state.activeCampaignId=state.campaigns[0]?.campaign?.id||'';renderCampaigns();}catch(e){message(e.message);}}
  function assetActions(asset){if(asset.status==='draft')return '<button type="button" data-promo-action="approve">Approve</button><button type="button" data-promo-action="reject">Reject</button>';if(asset.status==='approved')return '<input data-promo-schedule type="datetime-local"><button type="button" data-promo-action="schedule">Schedule</button><button type="button" data-promo-action="publish">Mark published</button>';if(asset.status==='scheduled')return '<button type="button" data-promo-action="publish">Mark published</button>';return `<span class="muted">${esc(asset.status)}</span>`;}
  function renderCampaigns(){const target=$('#promotion-campaigns');if(!target)return;if(!state.campaigns.length){target.innerHTML='<p class="muted">No durable promotion campaigns yet.</p>';return;}target.innerHTML=state.campaigns.map((entry)=>{const c=entry.campaign;return `<article class="memory promotion-campaign" data-campaign="${esc(c.id)}"><strong>${esc(c.objective)}</strong><p>${esc(c.audience)} • ${esc(c.readerPromise)}</p>${(c.assets||[]).map((a)=>`<div class="memory" data-asset="${esc(a.id)}"><strong>${esc(a.channel)} • ${esc(a.status)} • ${esc(a.title)}</strong><p>${esc(a.body)}</p>${(a.evidence||[]).map((e)=>`<small>${esc(e.confidence)} • ${esc(e.claim)}</small>`).join('')}<div class="row">${assetActions(a)}</div></div>`).join('')}</article>`;}).join('');$$('[data-promo-action]',target).forEach((button)=>button.addEventListener('click',promotionAction));}
  ...
  async function releaseGate(){const campaignId=state.activeCampaignId||state.campaigns[0]?.campaign?.id;if(!campaignId)return message('A promotion campaign is required for the combined release gate.');const format=readinessFormat();try{const report=await api(rootUrl(`/release-gate?bookId=${encodeURIComponent(state.bookId)}&format=${encodeURIComponent(format)}&campaignId=${encodeURIComponent(campaignId)}`));const target=$('#release-gate-result');target.innerHTML=`<article class="memory"><strong>${report.status==='ready'?'READY TO RELEASE':'RELEASE BLOCKED'}</strong><p>${esc(format)} • ${report.blockers.length} blocker(s)</p></article>`+(report.blockers||[]).map((b)=>`<article class="memory"><strong>${esc(b.kind)}</strong><p>${esc(b.message)}</p><small>${esc(b.remediation)}</small></article>`).join('');message(report.status==='ready'?'Combined Publishing + Promotion release gate is ready.':'Release blockers remain.',report.status==='ready');}catch(e){message(e.message);}}
  ...
  function bind(){
    ...
    $('#publishing-book')?.addEventListener('change',(e)=>void selectBook(e.target.value));
    $('#market-book')?.addEventListener('change',(e)=>void selectBook(e.target.value));
    $('#publishing-release-format')?.addEventListener('change',()=>void loadPublishingReadiness());
