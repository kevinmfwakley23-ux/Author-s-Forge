/* Forge Agent Workbench v3: server-discovered governed orchestration over real Forge APIs. */
(() => {
  "use strict";
  ...
  const plannerInput = $("#agent-planner");
  ...
  async function serverPlan(goal, recipeId = "") {
    ...
    const payload = { goal, ...target, ...(recipeId ? {} : { planner: plannerInput?.value || "deterministic" }) };
  ...
  function plannerSummary(response) {
    if (response.plannerUsed === "ai") return `AI-enhanced plan validated through ${response.plannerProvider || "configured provider"} / ${response.plannerModel || "selected model"}.`;
    if (response.plannerUsed === "deterministic-fallback") return `AI planner unavailable/invalid; Forge used deterministic fallback. ${response.plannerFallbackReason || "No valid AI plan was available."}`;
    if (response.plannerUsed === "recipe") return "Forge Recipe compiled through the deterministic governance boundary.";
    return "Deterministic provider-free planner used.";
  }
  ...
  async function saveModeAndPlan(event) {
    ...
    try {
      ...
      setStatus(`${plan.length} governed workflow steps planned. ${plannerSummary(planned.response)} Nothing has executed yet.`);
