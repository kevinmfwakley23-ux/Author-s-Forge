...
(() => {
  ...
  function ensureRecoveryPanel() {
    const versions = $('#versions'), grid = versions?.querySelector('.grid'); if (!grid || $('#project-recovery-card')) return;
    const card = document.createElement('article'); card.id = 'project-recovery-card'; card.className = 'card';
    card.innerHTML = `
  ...
  async function describeSelectedPackage() {
    const input = $('#restore-project-file'), file = input?.files?.[0], status = $('#restore-project-status'); if (!status) return;
    if (!file) { status.textContent = 'No recovery package selected.'; return; }
    try {
      const pkg = JSON.parse(await file.text()), manifest = pkg && typeof pkg === 'object' && !Array.isArray(pkg) ? pkg.manifest : null;
    ...
    } catch (error) { status.textContent = error instanceof Error ? error.message : 'Selected file is not valid JSON.'; }
  }

  async function restoreProject() {
    const input = $('#restore-project-file'), approval = $('#restore-project-confirm'), button = $('#restore-project'), status = $('#restore-project-status'), file = input?.files?.[0];
    ...
    try {
      let pkg; try { pkg = JSON.parse(await file.text()); } catch { throw new Error('Selected recovery package is not valid JSON.'); }
      ...
      if (!confirmed) { if (status) status.textContent = 'Restore cancelled. No project state was changed.'; return; }
      const result = await api(projectUrl('/package/restore'), { method: 'POST', body: JSON.stringify({ authorApproved: true, package: pkg }) });
      ...
      input.value = ''; approval.checked = false; message('Project restored from the validated package. Rollback package downloaded.', true); $('#refresh')?.click();
    } catch (error) {
      const text = error instanceof Error ? error.message : 'Project recovery failed.'; if (status) status.textContent = text; message(text);
    } finally { if (button) button.disabled = false; }
  }

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', ensureRecoveryPanel, { once: true }); else ensureRecoveryPanel();
})();
