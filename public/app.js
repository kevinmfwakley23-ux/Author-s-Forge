(() => {
  ...
  function navigate(route) {
    const target = document.getElementById(route) || document.getElementById("dashboard");
    if (!target) return;
  ...
  function renderPipeline(project) {
    ...
    $$(`[data-pipeline]`).forEach((button) => button.addEventListener("click", () => navigate(button.dataset.route)));
  }
  ...
  function bindNavigation() { $$('[data-route]').forEach((link) => link.addEventListener("click", (event) => { event.preventDefault(); navigate(link.dataset.route); })); }
  ...
  window.addEventListener("DOMContentLoaded", () => { navigate(location.hash.slice(1) || "dashboard"); refresh(); });
})();
