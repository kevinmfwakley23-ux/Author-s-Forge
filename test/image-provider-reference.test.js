...
function successResponse(){return new Response(JSON.stringify({data:[{b64_json:"QUJDRA=="}]}),{status:200,headers:{"content-type":"application/json","x-request-id":"req-image-1"}});}

test("image provider uses JSON generation endpoint when no approved visual reference exists",async()=>{
  ...
  assert.equal(call.url,"https://api.openai.com/v1/images/generations");assert.equal(call.init.method,"POST");assert.equal(call.init.headers["content-type"],"application/json");assert.equal(JSON.parse(call.init.body).model,"gpt-image-2");assert.equal(result.dataUri,"data:image/png;base64,QUJDRA==");
});

test("approved visual reference uses multipart image-edit path with actual image bytes",async()=>{
  let call;const fetchImpl=async(url,init)=>{call={url:String(url),init};return successResponse();};
  await generateImage({prompt:"Evolve this original guardian while preserving identity",size:"1024x1536",referenceImages:[{dataUri:"data:image/png;base64,QUJDRA==",label:"Approved guardian design"}]},{apiKey:"test-key",model:"gpt-image-2",fetchImpl});
  ...
  assert.ok(call.init.body instanceof FormData);assert.equal(call.init.body.get("model"),"gpt-image-2");assert.equal(call.init.body.get("size"),"1024x1536");assert.equal(call.init.body.has("input_fidelity"),false);const images=call.init.body.getAll("image[]");assert.equal(images.length,1);assert.ok(images[0] instanceof Blob);assert.equal(images[0].type,"image/png");assert.equal(images[0].size,4);
});

test("image provider rejects URL references and malformed inline images before network",async()=>{
...
test("image provider enforces reference count and byte limits before network",async()=>{
  const tiny={dataUri:"data:image/png;base64,QUJDRA=="};let calls=0;const fetchImpl=async()=>{calls++;return successResponse();};
