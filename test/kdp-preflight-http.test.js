...
function coverPlan(projectId, bookId, id, version = 1) {
  return new BookCoverStudioService().create({
    id,
    projectId,
    bookId,
    format: "paperback",
    publishing,
    title: `Production Book ${bookId}`,
    author: "Author",
    frontPrompt: "Front",
    spineText: `Production Book ${bookId}`,
    backText: "Back",
    outputFormat: "pdf",
    dpi: 300,
    version,
...
async function fixture(root, projectId = "project-a", includePlan = true) {
  ...
  if (includePlan) project = withProjectBookCoverPlans(project, [coverPlan(projectId, "book-1", "cover-1")]);
...
test("KDP preflight refuses ambiguous multi-book plan selection", async () => {
  const root = await mkdtemp(join(tmpdir(), "forge-kdp-http-"));
  try {
    const deps = await fixture(root);
    const project = await deps.projectStore.load("project-a");
    assert.ok(project);
    await deps.projectStore.save(withProjectBookCoverPlans(project, [
      ...(project.bookCoverPlans || []),
      coverPlan("project-a", "book-2", "cover-2"),
    ]));
    const input = payload();
    delete input.bookId;
    await assert.rejects(() => runKdpPreflightFromHttp(deps, input), /Multiple books have KDP cover plans/);
