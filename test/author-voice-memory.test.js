...
const { buildAuthorVoiceContext, createAuthorVoiceMemory, assessVoiceDrift, updateAuthorVoiceMemory } = require("../.forge-build/index.js");
...
test("author voice memory aggregates approved corpus and preserves canonical samples", () => {
  const memory = createAuthorVoiceMemory({ projectId: "p1", authorId: "a1", samples: [
    { id: "s1", label: "chapter sample", text: SAMPLE_A, weight: 2, source: "author", purpose: "prose" },
    { id: "s2", label: "memoir sample", text: SAMPLE_B, weight: 1, source: "approved-manuscript", purpose: "narration" },
  ] });
  assert.equal(memory.formatVersion, 3);
  ...
  assert.equal(memory.samples[0].source, "author");
  assert.ok(memory.dimensions.sentenceRhythm >= 0);
...
test("author voice memory supports corpus updates without losing provenance", () => {
  ...
  const updated = updateAuthorVoiceMemory(memory, { addSamples: [{ id: "s2", label: "two", text: SAMPLE_B, weight: 2, source: "approved-manuscript", genre: "memoir" }], canonicalSampleIds: ["s2"] });
  assert.equal(updated.samples.length, 2);
  assert.deepEqual(updated.canonicalSampleIds, ["s2"]);
  assert.equal(updated.samples[1].source, "approved-manuscript");
  assert.equal(updated.samples[1].genre, "memoir");
  assert.notEqual(updated.updatedAt, memory.updatedAt);
});

test("voice drift assessment returns actionable warnings and dimension guidance", () => {
  const memory = createAuthorVoiceMemory({ projectId: "p1", authorId: "a1", samples: [{ id: "s1", label: "one", text: SAMPLE_A }] });
  ...
  assert.ok(Object.hasOwn(report.dimensions, "dialogue"));
});
