...
function projectWithPlan() {
  ...
  const plan = new BookCoverStudioService().create({ id: "cover-1", projectId: "project-1", bookId: "book-1", format: "paperback", publishing, title: "Production Book", author: "Author", frontPrompt: "Front", spineText: "Production Book", backText: "Back", outputFormat: "pdf", dpi: 300, version: 1, approvalStatus: "draft", now: "2026-08-31T20:01:00.000Z" });
...
async function withService(run) {
  ...
  try {
    const studio = new StudioKdpPreflightService(new KdpPreflightHistoryService(new FileKdpPreflightStore(join(dir, "preflight.json"))));
    await run(studio);
  } finally {
...
test("Studio KDP preflight uses authoritative durable cover geometry", async () => {
  await withService(async (studio) => {
    const project = projectWithPlan();
    const f = facts();
    const result = await studio.audit({ project, bookId: "book-1", assertedPublishing: publishing, interiorHasBleed: false, ...f, reportId: "report-1", now: "2026-08-31T20:10:00.000Z" });
    assert.equal(result.coverPlanId, "cover-1");
    assert.equal(result.bookId, "book-1");
    assert.equal(result.publishing.trimWidthInches, 6);
    assert.equal(result.report.status, "ready");
    assert.equal((await studio.latest("project-1")).id, "report-1");
  });
});

test("Studio KDP preflight rejects caller publishing geometry that disagrees with Cover Studio", async () => {
  await withService(async (studio) => {
    const project = projectWithPlan();
    const f = facts();
    await assert.rejects(
      () => studio.audit({ project, coverPlanId: "cover-1", assertedPublishing: { ...publishing, trimWidthInches: 7 }, interiorHasBleed: false, ...f }),
      /disagrees with the durable Cover Studio plan \(trimWidthInches\)/,
    );
    assert.equal((await studio.list("project-1")).length, 0);
  });
});

test("Studio KDP preflight still blocks a physical file whose measured cover geometry is wrong", async () => {
  await withService(async (studio) => {
    const project = projectWithPlan();
    const f = facts();
...
test("Studio KDP preflight requires a durable KDP cover plan", async () => {
  await withService(async (studio) => {
