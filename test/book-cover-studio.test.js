...
test("Mission 015 calculates a KDP paperback exterior from publishing configuration", () => {
  ...
  assert.equal(dimensions.widthInches, 12.8012);
...
test("Mission 015 creates and validates a complete paperback cover plan", () => {
  ...
  assert.equal(plan.dimensions.widthInches, 12.8012);
  assert.equal(plan.zones.front.height, 9);
  const issues = service.validate("cover-1", { format: "pdf", widthInches: 12.8012, heightInches: 9.25, dpi: 300, sizeBytes: 1000000, hasFront: true, hasBack: true, hasSpine: true, hasCropMarks: false, hasTemplateText: false, flattened: true, fontsEmbedded: true, encrypted: false });
  assert.deepEqual(issues, []);
});

test("Mission 015 rejects production files that violate publishing requirements", () => {
  const plan = { id: "cover", projectId: "p", bookId: "b", format: "paperback", publishing: { platform: "kdp", binding: "paperback", interiorType: "black-white", paperType: "white", trimWidthInches: 6, trimHeightInches: 9, pageCount: 100, bleedInches: 0.125, readingDirection: "ltr" }, dimensions: calculateKdpCoverLayout({ platform: "kdp", binding: "paperback", interiorType: "black-white", paperType: "white", trimWidthInches: 6, trimHeightInches: 9, pageCount: 100, bleedInches: 0.125, readingDirection: "ltr" }).dimensions, zones: calculateKdpCoverLayout({ platform: "kdp", binding: "paperback", interiorType: "black-white", paperType: "white", trimWidthInches: 6, trimHeightInches: 9, pageCount: 100, bleedInches: 0.125, readingDirection: "ltr" }).zones, title: "x", author: "y", frontPrompt: "x", spineText: "x", backText: "x", outputFormat: "pdf", dpi: 300, version: 1, approvalStatus: "draft", createdAt: "2026-01-01T00:00:00.000Z", updatedAt: "2026-01-01T00:00:00.000Z" };
