...
async function handleApi(req: IncomingMessage, res: ServerResponse, url: URL): Promise<boolean> {
  ...
  if (projectId) {
    ...
    if (url.pathname === `/api/projects/${projectId}/memory` && req.method === "POST") {
      ...
      const memory = createMemoryRecord({ id: String(input.id ?? `memory-${randomUUID()}`), projectId, class: enumValue(input.class ?? "creative-note", MEMORY_CLASSES, "memory class"), authority: enumValue(input.authority ?? "working", MEMORY_AUTHORITIES, "memory authority"), summary: String(input.summary ?? ""), content: String(input.content ?? ""), provenance: [{ kind: "author", reference: String(input.reference ?? "studio"), recordedAt: new Date().toISOString() }], relatedMemoryIds: Array.isArray(input.relatedMemoryIds) ? input.relatedMemoryIds.map(String) : [], relevanceTags: Array.isArray(input.relevanceTags) ? input.relevanceTags.map(String) : [] });
      if (project.memories.some((m) => m.id === memory.id)) { json(res, 409, { error: `Memory id "${memory.id}" already exists.` }); return true; }
      const saved = { ...project, memories: [...project.memories, memory], metadata: { ...project.metadata, updatedAt: new Date().toISOString() } };
      await store.save(saved); json(res, 201, memory); return true;
    }
    if (url.pathname === `/api/projects/${projectId}/context` && req.method === "POST") {
      ...
      const policies = Array.isArray(input.policies) ? input.policies.map((item) => {
        if (!item || typeof item !== "object") throw new Error("Context policy must be an object.");
        const policy = item as Record<string, unknown>;
        const mode = enumValue(policy.mode, CONTEXT_INCLUSION_MODES, "context inclusion mode");
        const key = String(policy.key ?? "").trim();
        if (!key) throw new Error("Context policy key is required.");
        const maxWords = policy.maxWords === undefined ? undefined : Number(policy.maxWords);
        if (maxWords !== undefined && (!Number.isInteger(maxWords) || maxWords < 1 || maxWords > 100000)) throw new Error("Context maxWords must be an integer from 1 to 100000.");
        return maxWords === undefined ? { key, mode } : { key, mode, maxWords };
      }) as ContextSectionPolicy[] : undefined;
      const characterIds = Array.isArray(input.characterIds) ? input.characterIds.map(String) : undefined;
      json(res, 200, assembleWritingContext(project, { projectId, policies, query: input.query === undefined ? undefined : String(input.query), characterIds }));
      return true;
    }
...
async function serveStatic(req: IncomingMessage, res: ServerResponse, url: URL): Promise<void> {
  ...
  if (safe.startsWith("..") || safe.includes("/../")) { text(res, 400, "Bad path", "text/plain; charset=utf-8"); return; }
  const file = join(publicRoot, safe);
  try { const data = await readFile(file); const type: Record<string, string> = { ".html": "text/html; charset=utf-8", ".css": "text/css; charset=utf-8", ".js": "text/javascript; charset=utf-8", ".json": "application/json; charset=utf-8", ".svg": "image/svg+xml" }; res.writeHead(200, { "content-type": type[extname(file)] ?? "application/octet-stream", "cache-control": "no-cache", "x-content-type-options": "nosniff", "content-security-policy": "default-src 'self'; connect-src 'self'; img-src 'self' data:; style-src 'self' 'unsafe-inline'; script-src 'self'" }); res.end(data); }
  catch { text(res, 404, "Not found", "text/plain; charset=utf-8"); }
}

const server = createServer(async (req, res) => {
  try { const url = new URL(req.url ?? "/", `http://${req.headers.host ?? `${host}:${port}`}`); if (url.pathname.startsWith("/api/")) { if (await handleApi(req, res, url)) return; json(res, 404, { error: "API route not found." }); return; } if (req.method !== "GET" && req.method !== "HEAD") { json(res, 405, { error: "Method not allowed." }); return; } await serveStatic(req, res, url); }
