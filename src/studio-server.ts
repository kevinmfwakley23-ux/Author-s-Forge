...
import { BOOK_GENOME_COMPONENTS, DELIVERY_AUDIT_CATEGORIES as FINAL_AUDIT_CATEGORIES, type BookGenomeNode, type FinalDeliveryCheck } from "./domain/final-product-systems";
...
function json(res:ServerResponse,status:number,value:unknown):void { const body=JSON.stringify(value); res.writeHead(status,{"content-type":"application/json; charset=utf-8","cache-control":"no-store","x-content-type-options":"nosniff"}); res.end(body); }
...
async function handleApi(req:IncomingMessage,res:ServerResponse,url:URL):Promise<boolean> {
  ...
  if(url.pathname==="/api/projects"&&req.method==="POST"){const input=await body(req);const id=String(input.id??"");const title=String(input.title??"");const project=createProject({id,title});await store.create(project);json(res,201,project);return true;}
...
function cryptoRandomId(prefix:string):string{return `${prefix}-${Date.now()}-${Math.random().toString(36).slice(2,10)}`;}
