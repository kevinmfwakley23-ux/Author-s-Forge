...
export class FileProjectStore {
  ...
  private validate(value: unknown, expectedId: string): ProjectState {
    ...
    const candidate = value as Record<string, unknown>; const metadata = candidate.metadata;
    ...
    if (characters !== undefined) { if (!Array.isArray(characters)) throw new Error("Invalid project character state."); for (const character of characters) { const validated = validateCharacterRecord(character); if (validated.projectId !== expectedId) throw new Error("Project character state contains a character from another project."); } }
    const visualIdentities = candidate.visualIdentities === undefined ? undefined : candidate.visualIdentities;
    if (visualIdentities !== undefined) { if (!Array.isArray(visualIdentities)) throw new Error("Invalid project visual identity state."); for (const identity of visualIdentities) { const validated = validateVisualCharacterIdentity(identity); if (validated.projectId !== expectedId) throw new Error("Project visual identity state contains an identity from another project."); } }
