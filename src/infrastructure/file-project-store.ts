...
function validateOptional<T>(value: unknown, validator: (item: unknown) => T, expectedProjectId: string, label: string): T[] | undefined {
  if (value === undefined) return undefined; if (!Array.isArray(value)) throw new Error(`Invalid ${label.toLowerCase()} state.`); const seen = new Set<string>();
  const validateUnknown = validator as unknown as (item: unknown) => T;
  return value.map((item) => { const validated = validateUnknown(item) as T & { projectId?: unknown; id?: unknown }; if (validated.projectId !== expectedProjectId) throw new Error(`${label} belongs to another project.`); if (typeof validated.id === "string") { if (seen.has(validated.id)) throw new Error(`Duplicate ${label.toLowerCase()} id \"${validated.id}\".`); seen.add(validated.id); } return JSON.parse(JSON.stringify(validated)) as T; });
}
