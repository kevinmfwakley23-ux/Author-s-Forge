...
export function validateTcgGameFramework(value:TcgGameFramework):void {
  ...
  uniqueObjects(value.worldMaps,"TCG world map",map=>map.id,map=>validateWorldMap(map,value.characterLines));
  const territoryIds=new Set(value.worldMaps.flatMap(map=>map.territories.map(t=>t.id)));
...
export function withTcgCharacterLine(framework:TcgGameFramework,line:TcgCharacterEvolutionLine):TcgGameFramework {validateCharacterLine(line);const exists=framework.characterLines.some(item=>item.id===line.id);const next={...framework,characterLines:Object.freeze(exists?framework.characterLines.map(item=>item.id===line.id?clone(line):item):[...framework.characterLines,clone(line)])};validateTcgGameFramework(next);return Object.freeze(next);}
export function withTcgWorldMap(framework:TcgGameFramework,map:TcgWorldMap):TcgGameFramework {const exists=framework.worldMaps.some(item=>item.id===map.id);const next={...framework,worldMaps:Object.freeze(exists?framework.worldMaps.map(item=>item.id===map.id?clone(map):item):[...framework.worldMaps,clone(map)])};validateTcgGameFramework(next);return Object.freeze(next);}
