...
export function assessMarketingAssetCompliance(asset: MarketingAsset): readonly MarketingComplianceIssue[] {
  ...
  if (asset.channel === "amazon-ads") {
    if (/\$\s*\d|\bprice\b|\bon sale\b|\bdiscount\b/i.test(body)) issues.push({ id: "amazon-ads-price", severity: "error", message: "Amazon book ad custom text cannot include prices or promotional pricing.", remediation: "Remove price/discount language from ad creative." });
    if (/\bguarantee(?:d|e)?\b|\b#?1\b|\bbest[- ]?seller\b|\btop[- ]?rated\b/i.test(body) && !hasSupportingEvidence(asset)) issues.push({ id: "amazon-ads-unsubstantiated", severity: "error", message: "Amazon book ad copy contains an unsubstantiated claim or guarantee.", remediation: "Remove the claim or attach authoritative evidence where the policy permits substantiation." });
    if (/\bcustomer review\b|\bverified purchase\b|\bstars?\b/i.test(body)) issues.push({ id: "amazon-ads-reviews", severity: "error", message: "Amazon book ad custom text cannot include customer reviews or review-style star claims.", remediation: "Use accurate book-focused creative instead." });
  }
  if (asset.channel === "a-plus") {
    ...
    if (/\bcustomer review\b|\breviewer said\b|\bverified purchase\b/i.test(body)) issues.push({ id: "a-plus-review", severity: "error", message: "A+ Content cannot contain customer reviews or private-person review attributions.", remediation: "Remove customer-review material; only permitted publication/public-figure endorsements should be considered under current KDP rules." });
    ...
    if (/\bqr\s*code\b|\bbarcode\b/i.test(body)) issues.push({ id: "a-plus-qr", severity: "error", message: "A+ Content cannot contain QR/barcode redirect material.", remediation: "Remove QR/barcode references from the module." });
    if (/\bguarantee(?:d|e)?\b|\bwarranty\b|\brefund\b/i.test(body)) issues.push({ id: "a-plus-guarantee", severity: "error", message: "A+ Content cannot contain guarantees, warranties, or off-Amazon refund-policy claims.", remediation: "Remove guarantee, warranty, and refund-policy language." });
    if (/\bfree shipping\b|\bshipping\b|\bdelivery\b/i.test(body)) issues.push({ id: "a-plus-shipping", severity: "error", message: "A+ Content cannot contain shipping or delivery claims.", remediation: "Remove shipping and delivery language." });
    if (/\b(?:versus|vs\.?|better than|unlike)\b/i.test(body) || /\bcompetitor(?:s)?\b/i.test(body)) issues.push({ id: "a-plus-competitor", severity: "error", message: "A+ Content cannot compare the book with competitors.", remediation: "Describe this book on its own merits; use comparison charts only for eligible books in the same KDP account." });
    if (/\b#?1\b|\bbest[- ]?selling\b|\btop[- ]?rated\b|\b100%\s+satisfaction\b/i.test(body)) issues.push({ id: "a-plus-boastful", severity: "error", message: "A+ Content cannot contain boastful rankings or satisfaction claims.", remediation: "Remove ranking, top-rated, and satisfaction claims." });
  }
  return issues;
}
