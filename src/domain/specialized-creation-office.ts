...
export type SpecializedElementKind = "text" | "image" | "shape" | "group" | "panel" | "qr";
...
export type SpecializedArtifactKind = "pdf" | "svg" | "png" | "cbz" | "json" | "csv";
...
export interface ComicData { readonly issueTitle:string; readonly issueNumber?:string; readonly pages:readonly {page:number; pageTurnIntent?:string; panels:readonly ComicPanelData[]}[]; }
...
export function defaultProductionProfile(mode:SpecializedCreationMode):SpecializedProductionProfile {
  ...
  if(mode==="comic-book") return Object.freeze({...common,id:"comic-standard",label:"Comic 6.625 × 10.25",widthInches:6.625,heightInches:10.25,artifactKinds:["pdf","cbz","svg"] as SpecializedArtifactKind[]});
  ...
  if(mode==="flyer") return Object.freeze({...common,id:"flyer-letter",label:"US Letter Flyer",widthInches:8.5,heightInches:11,artifactKinds:["pdf","svg","png"] as SpecializedArtifactKind[]});
...
export function emptyModeData(mode:SpecializedCreationMode):SpecializedModeData {
  if(mode==="comic-book") return Object.freeze({issueTitle:"",pages:[]}) as ComicData;
...
export function validateModeData(mode:SpecializedCreationMode,data:SpecializedModeData):void {
  if(mode==="comic-book"){const comic=data as ComicData;const pages=new Set<number>();for(const page of comic.pages){if(!Number.isInteger(page.page)||page.page<1)throw new Error("Comic page number must be positive.");if(pages.has(page.page))throw new Error(`Duplicate comic page ${page.page}.`);pages.add(page.page);const orders=new Set<number>();for(const panel of page.panels){if(panel.page!==page.page)throw new Error("Comic panel page mismatch.");if(orders.has(panel.order))throw new Error(`Duplicate comic panel order ${panel.order} on page ${page.page}.`);orders.add(panel.order);}}return;}
  ...
  if(mode==="trading-card-game"){const tcg=data as TcgData;const fields=new Set<string>();for(const field of tcg.fields){required(field.key,"TCG field key");if(fields.has(field.key))throw new Error(`Duplicate TCG field \"${field.key}\".`);fields.add(field.key);}const ids=new Set<string>(),numbers=new Set<string>();for(const card of tcg.cards){required(card.id,"TCG card id");required(card.collectorNumber,"TCG collector number");if(ids.has(card.id))throw new Error(`Duplicate TCG card id \"${card.id}\".`);if(numbers.has(card.collectorNumber))throw new Error(`Duplicate collector number \"${card.collectorNumber}\".`);ids.add(card.id);numbers.add(card.collectorNumber);for(const field of tcg.fields.filter(f=>f.required)){const value=card.fields[field.key];if(value===undefined||value===null||String(value).trim()==="")throw new Error(`Card \"${card.id}\" missing required field \"${field.key}\".`);}}return;}
