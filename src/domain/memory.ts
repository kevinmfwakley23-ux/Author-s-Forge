...
function normalizeProvenance(provenance: readonly MemoryProvenance[]): readonly MemoryProvenance[] {
  return provenance.map((item) => { if (!item.reference.trim()) throw new Error("Memory provenance reference is required."); if (!item.recordedAt.trim()) throw new Error("Memory provenance timestamp is required."); return { ...item, reference: item.reference.trim() }; });
}
