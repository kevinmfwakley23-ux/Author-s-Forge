...
export function createPublishingReadinessReport(input: PublishingReadinessInput & { id: string; projectId: string; bookId?: string; now?: string }): PublishingReadinessReport {
  ...
  const print = c?.format === "paperback" || c?.format === "hardcover";
  const digital = c?.format === "ebook" || c?.format === "audiobook";
  ...
  const checks: ReadinessCheck[] = [
    ...
    check("toc","table-of-contents","Table of contents",!tocRequired || !!m?.hasTableOfContents,!tocRequired ? "Table of contents is explicitly not required for this book." : "Table of contents is present.","Generate and validate the table of contents, or explicitly mark it not applicable."),
    ...
    check("cover-back","cover","Back cover",digital || !!c?.hasBack,digital ? "Back cover is not required for this digital format." : "Back cover is present.","Provide the back cover for print covers."),
    check("cover-spine","cover","Spine",digital || !!c?.hasSpine,digital ? "Spine is not required for this digital format." : "Spine is present.","Provide the spine for print covers."),
    check("barcode-safe","cover","Barcode-safe area",digital || !!c?.hasBarcodeSafeArea,digital ? "Barcode-safe area is not required for this digital format." : "Barcode-safe area is satisfied.","Reserve the barcode-safe area."),
    ...
    check("cover-dimensions","production","Cover dimensions",!print || (!!c?.widthInches && !!c?.heightInches && c.widthInches > 0 && c.heightInches > 0),print ? "Print cover dimensions are valid." : "Print cover dimensions are not required for this digital release.","Provide positive final print-cover dimensions."),
    ...
    check("page-numbering","pagination","Page numbering",!print || !!f?.pageNumbering,print ? "Print page numbering is configured." : "Fixed print page numbering is not required for this digital release.","Configure print page numbering."),
    check("headers-footers","formatting","Headers and footers",!print || !!f?.headersFooters,print ? "Headers/footers are configured." : "Fixed print headers/footers are not required for this digital release.","Configure or explicitly omit print headers/footers.","warning"),
    check("images-present","images","Images",!imagesRequired || (!!i?.count && i.count > 0),imagesRequired ? "Required image assets are present." : "Images are explicitly not required for this book.","Add the required image assets.","warning"),
