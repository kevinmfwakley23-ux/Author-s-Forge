...
export type TcgDataWithFramework=TcgData&{readonly gameFramework?:TcgGameFramework};

export function tcgGameFramework(data:TcgData):TcgGameFramework {const framework=(data as TcgDataWithFramework).gameFramework??emptyTcgGameFramework();validateTcgGameFramework(framework);return clone(framework);}
export function setTcgGameFramework(data:TcgData,framework:TcgGameFramework):TcgDataWithFramework {validateTcgGameFramework(framework);return Object.freeze({...clone(data),gameFramework:clone(framework)});}
...
export function createCharacterEvolutionCards(data:TcgData,input:{lineId:string;templateId:string;collectorPrefix?:string}):TcgDataWithFramework {
  ...
  const created:TcgCardRecord[]=[];for(const stage of [...line.stages].sort((a,b)=>a.stageIndex-b.stageIndex)){
    ...
    created.push(Object.freeze({id,collectorNumber,fields:Object.freeze(fields),...(stage.approvedArtworkAssetId?{artworkAssetId:stage.approvedArtworkAssetId}:{}),templateId:input.templateId}));existingIds.add(id);existingNumbers.add(collectorNumber);
  }
