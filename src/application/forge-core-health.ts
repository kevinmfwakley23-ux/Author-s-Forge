...
export interface ForgeCoreHealthReport {
  ...
  readonly operationalAiModels: number;
...
export function inspectForgeCore(core: ForgeCore, now = new Date().toISOString()): ForgeCoreHealthReport {
  ...
  const checks = [
    ...
    { name: 'ai-operational-capacity', ok: readiness.aiOperational, detail: readiness.aiOperational ? `${readiness.operationalModelCount} operational model(s)` : 'configured AI resources are unhealthy, cooling down, or unavailable' },
    ...
    { name: 'ai-execution', ok: readiness.aiRoutingAvailable, detail: readiness.aiRoutingAvailable ? 'shared broker-driven retry/failover boundary available' : 'shared AI execution boundary unavailable' }
  ];
  ...
  return {
    ...
    operationalAiModels: readiness.operationalModelCount,
    checks
  };
}
