...
export class AiExecutionFallback {
 ...
 private isRetryable(error:unknown):boolean{
  if(error&&typeof error==='object'&&'retryable' in error){
   const explicit=(error as {retryable?:unknown}).retryable;
   if(typeof explicit==='boolean')return explicit;
  }
  return /timeout|timed out|temporar|rate.?limit|429|503|502|504|overloaded|unavailable|network|connection|econn|reset|quota|forge quality gate/i.test(this.message(error).toLowerCase());
 }
}
