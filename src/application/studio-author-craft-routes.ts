...
import { createStudioAiControlRoutes } from "./studio-ai-control-routes";
...
export function createStudioAuthorCraftRoutes(store: FileProjectStore): StudioAuthorCraftRouteHandler {
  const aiControl = createStudioAiControlRoutes(store);
  ...
  return async (req, res, url, projectId) => {
    if (await aiControl(req, res, url, projectId)) return true;
