...
export function reorderComicPanels(data:ComicData,pageNumber:number,panelIds:readonly string[]):ComicProductionData {
  ...
  const byId=new Map(page.panels.map(panel=>[panel.id,panel]));if(panelIds.some(id=>!byId.has(id)))throw new Error("Comic reorder references an unknown panel.");
  const panels=panelIds.map((id,index)=>Object.freeze({...byId.get(id),order:index+1}));
  return Object.freeze({...data,...(extended.readingDirection?{readingDirection:extended.readingDirection}:{}),pages:Object.freeze(data.pages.map(item=>item.page===pageNumber?Object.freeze({...item,panels:Object.freeze(panels)}):item))});
}
...
export function createFoldedCardImpositionDocument(project:SpecializedOfficeProject,source:SpecializedDocument,now=new Date().toISOString()):SpecializedDocument {
  ...
  const profile=foldedCardImpositionProfile(project.productionProfiles[0]!);const front=findSurface(source,"front"),back=findSurface(source,"back"),insideLeft=findSurface(source,"inside-left"),insideRight=findSurface(source,"inside-right");
...
export function createTcgSheetDocument(project:SpecializedOfficeProject,source:SpecializedDocument,duplexFlip:DuplexFlip="long-edge",now=new Date().toISOString()):SpecializedDocument {
  ...
  const data=project.modeData as TcgData;const cardSurfaces=source.surfaces.filter(surface=>surface.kind==="card-front");if(!cardSurfaces.length)throw new Error("TCG sheet requires rendered card fronts.");
  const profile=tcgSheetProfile(project.productionProfiles[0]!,duplexFlip),surfaces:SpecializedSurface[]=[];
