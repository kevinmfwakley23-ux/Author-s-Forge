...
export class ProjectPackageBackupService {
  ...
  public async backupExisting(
    ...
    return { projectId: validatedProjectId, backupId: normalizedBackupId, key, stored, package: pkg };
  }

  public async listBackups(projectId: string, binding: ProjectStorageBinding): Promise<readonly StoredObject[]> {
    const validatedProjectId = validateForgeProjectId(projectId);
    const validatedBinding = this.bindingForProject(validatedProjectId, binding);
    const objects = await this.storage.list(validatedBinding, "backups");
    return [...objects]
      .filter((item) => item.key.endsWith(".forge-project.json"))
...
function backupObjectKey(value: unknown): string {
  ...
  if (!key.startsWith("backups/") || !key.endsWith(".forge-project.json")) {
    throw new Error("Project backup key must reference a Forge backup object under backups/.");
  }
  return key;
}
